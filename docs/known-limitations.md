...
| **Ollama extraction quality** | The model itself *is* now verified: `qwen3.5:2b` was pulled on 2026-07-28 and the `@live` tier passed against it. What is poor is its usefulness — with thinking disabled and temperature 0 it refused every conversational phrasing tried, so the deterministic rules parser does all the useful work and the model adds ~4 s of latency without adding capability. That is a property of a 2.3 B model, not of the integration, and it is why the workflow is built to run correctly on `LLM_PROVIDER=rules`. A larger model would need re-measuring, not re-coding. |
