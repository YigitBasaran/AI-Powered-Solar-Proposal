    ...
    <Card className="px-3.5 py-3">
      ...
      >
        <div className="flex flex-wrap items-baseline gap-x-5 gap-y-1 text-[12.5px]">
          <span className="text-slate-muted">
            ...
            <strong
              data-testid="fx-capex-usd"
              className="tnum font-semibold text-slate-ink"
            >
          ...
          <span className="text-slate-muted">
            ...
            <span
              data-testid="fx-rate"
              className="tnum font-medium text-slate-ink"
            >
              {fx.rate}
            </span>{" "}
            {fx.quoteCurrency}
      ...
      {fx.isFixture ? (
        <p className="mt-2 text-[11.5px] text-[#8a5210]">
          This rate is demo fixture data, not a live market rate. USD and EUR
          are never treated as equal — the conversion above is always applied.
        </p>
      ) : null}
    </Card>
  );
    ...
    <Card className="px-3.5 py-3">
      ...
      >
        {energy.facets.map((facet) => (
          ...
          >
            <td className="px-2 py-1.5">
              {labels.get(facet.facetId) ?? facet.facetId}
            </td>
            <td className="tnum px-2 py-1.5 text-right">{facet.panelCount}</td>
            <td className="tnum px-2 py-1.5 text-right">
              {facet.installedPowerKwp.toFixed(1)} kWp
            </td>
            <td className="tnum px-2 py-1.5 text-right">
              {degrees(facet.pvgisAspectDeg)}
            </td>
            <td className="tnum px-2 py-1.5 text-right">
        ...
        <tr className="font-semibold">
          ...
          <td className="tnum px-2 py-1.5 text-right">
            {energy.installedPowerKwp.toFixed(1)} kWp
          </td>
          <td />
          <td />
          <td className="tnum px-2 py-1.5 text-right">
            {kwh(energy.totalAnnualProductionKwh)}
          </td>
        </tr>
      </DataTable>

      <p className="mt-2 text-[11px] text-slate-muted">
        One PVGIS request per occupied facet, using that facet&apos;s own pitch
        and azimuth.
        {energy.radiationDatabase
          ? ` Radiation database: ${energy.radiationDatabase}.`
          : ""}
      </p>
    </Card>
  );
  ...
  return (
    <Card className="px-3.5 py-3">
      ...
      <div className="mt-3 grid gap-x-8 gap-y-1 text-[12.5px] sm:grid-cols-2">
        <Row
          label="Annual production"
          value={kwh(financial.annualProductionKwh)}
        />
        <Row
          label="Annual consumption"
          value={kwh(financial.annualConsumptionKwh)}
        />
        ...
        />
        <Row
          label="Capital cost (as quoted)"
          value={usd(financial.originalCapex.amount)}
        />
        <Row
          label="Capital cost (converted)"
          value={eur(financial.convertedCapex.amount)}
        />
      </div>

      <p className="mt-2 text-[11px] text-slate-muted">
        Savings are capped at consumption: the case grants no export
        compensation. Flat tariff, no degradation, inflation, maintenance or
        financing cost.
      </p>
    </Card>
  );
...
export function RoofSection({ analysis }: { analysis: Analysis }) {
  ...
  return (
    <Card className="px-3.5 py-3">
      <SectionTitle
        action={
          <span className="text-[11.5px] text-slate-muted">
            {area(roof.totalProjectedAreaM2)} plan ·{" "}
            {area(roof.totalSlopedAreaM2)} sloped
          </span>
        }
      >
        Roof measurements
      ...
      >
        {roof.facets.map((facet) => (
          ...
          >
            ...
            <td className="tnum px-2 py-1.5 text-right">
              {area(facet.projectedAreaM2)}
            </td>
            <td className="tnum px-2 py-1.5 text-right">
              {area(facet.slopedAreaM2)}
            </td>
            <td className="tnum px-2 py-1.5 text-right">
              {panels.get(facet.id) ?? 0}
            </td>
          </tr>
        ))}
      </DataTable>
      ...
      >
        {roof.edges.map((edge) => (
          ...
          >
            ...
            <td className="px-2 py-1.5 capitalize text-slate-body">
              {edge.type}
            </td>
            <td className="tnum px-2 py-1.5 text-right">
              {edge.projectedLengthM.toFixed(3)} m
            </td>
            <td className="tnum px-2 py-1.5 text-right">
              {edge.true3dLengthM !== null
                ? `${edge.true3dLengthM.toFixed(3)} m`
                : "—"}
            </td>
          </tr>
        ))}
      ...
      <p className="mt-2 text-[11px] text-slate-muted">
        Hip lengths use true 3-D endpoint geometry. They are deliberately not
        the plan length divided by cos(pitch), which applies only to an edge
        running straight up the slope.
      </p>
    </Card>
  );
}
